...
class CustomerModel{
	...
	function getCustomerById($con, $cusId){
		...
		$stmt = $con->prepare("SELECT * FROM customer c, cus_login cl WHERE c.cus_id=cl.cus_id AND c.cus_id=?");
	...
	function cusTelEdit($con, $newtel,$cus_id){
		if($con->errorCode() != null){
			if($con->errorCode() != '00000'){
				return 0;
			}
		...
		$stmt =$con->prepare("UPDATE customer SET cus_tel=? WHERE cus_id=? ");
	
		$stmt->execute(array($newtel, $cus_id));
	...
	function updateImageName($con, $imageName, $cusId){
		if($con->errorCode() != null){
			if($con->errorCode() != '00000'){
				return false;
			}
		...
		$stmt = $con->prepare("UPDATE cus_login SET cus_image=? WHERE cus_id=?");
		$stmt->execute(array($imageName, $cusId));
	}
