...
class CustomerController{
	...
	function cuseditImage($data, $image){
		...
		if ($image['file-input']['name'] != "") {
			...
			$fileNameToArray = explode(".", $image["file-input"]["name"]);
			$extension = end($fileNameToArray);

			$imageName = $data['customerId'] . '.' . $extension;

			
			if(move_uploaded_file($image['file-input']['tmp_name'], $path . $imageName)){
				$this->customerModel->updateImageName($this->con, $imageName, $data['customerId']);
			}else{
				return 0;
			}
