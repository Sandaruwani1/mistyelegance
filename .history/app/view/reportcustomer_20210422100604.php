...
<body>
    ...
    <div class="row">
        <div class="col-sm-6">
            <div class="card w-100">
                <div class="card-body">
                    ...
                    <h3 class="card-title" style="text-align: center;">Customer Analysis</h3>

                
                    <label style="font-weight:bold; text-align:left"> From: </label>
                        
                        <input type="date" id="report-from-customer-analysis" class="form-control"  max="<?php echo date('Y-m-d'); ?>" />
                        <span id="report-from-customer-analysis-error" class="text text-danger"></span>
                
                        <div class="clearfix">&nbsp;</div>
                        <label style="font-weight: bold;"> Until: </label>
                        
                        <input type="date" id="report-until-customer-analysis" class="form-control" max="<?php echo date('Y-m-d'); ?>" />
                        <span id="report-until-customer-analysis-error" class="text text-danger"></span>
                        <div class="clearfix">&nbsp;</div>

                        <button class="btn btn-primary" onclick="createCustomercountReport()">View Report</button>
     ...
     <div class="col-sm-6">
        ...
        <div class="card-body text">
            <h3 class="card-title" style="text-align: center;"> Customer Package reservations For Year</h3>
            <div class="clearfix">&nbsp;</div>
            <div class="row">
            <div class="col-sm-6"> 
                               
                        <label style="font-weight:bold; text-align:left"> Customer Id </label>   
                          <input autocomplete="off" type="text" id="cus-nic" name="cus-nic" placeholder="Customer NIC" class="form-control" required>
        ...
        <button class="btn btn-primary" onclick="createCustomerAnalysisReport()">View Report</button>
    ...
    <script>
        ...
        function createCustomerAnalysisReport(){
            var cusId = $("#cus-nic").val();
            var year = $("#year").val();
            if(cusId.length < 3 || $("#result").is(":visible")){
                $("#cus-nic-error").text("Customer ID cannot be empty!");
                $("#cus-nic").focus();
                return false;
            }
            
            window.open(`reportpdf.php?type=customer_analysis&cusId=${cusId}&year=${year}`, '_blank');
        }

        function createCustomercountReport(){
            var fromDate = $("#report-from-customer-analysis").val();
            var untilDate = $("#report-until-customer-analysis").val();
            if(fromDate == ""){
                $("#report-from-customer-analysis-error").text("From date cannot be empty!");
                $("#report-from-customer-analysis").focus();
                return false;
            }
            if(untilDate == ""){
                $("#report-until-customer-analysis-error").text("Until date cannot be empty!");
                $("#report-until-customer-analysis").focus();
                return false;
            }
            window.open(`reportpdf.php?type=customer_analysisCount&from=${fromDate}&until=${untilDate}`, '_blank');
        }

        $("#report-until-customer-analysis").change(function(){
            $("#report-until-customer-analysis-error").text("");
        });
        $("#report-from-customer-analysis").change(function(){
            $("#report-from-customer-analysis-error").text("");
        });
        $("#cus-nic").keyup(function(){
            $("#cus-nic-error").text("");
        });
