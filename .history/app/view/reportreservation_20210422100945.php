...
<body>
                        ...
                        <div class="page-header">
                            <div class="page-title">
                                <ol class="breadcrumb text-left">
                                    ...
                                    <li class="active">Report Reservation</li>
                                </ol>
                            </div>
                        </div>
                    </div>
    ...
    <div class="row">
        <div class="col-sm-6">
            <div class="card w-100">
                <div class="card-body">
                    ...
                    <h3 class="card-title" style="text-align: center;">Reservation Analysis</h3>

                
                    <label style="font-weight:bold; text-align:left"> From: </label>
                        
                        <input type="date" id="report-from-res-analysis" class="form-control"  max="<?php echo date('Y-m-d'); ?>" />
                        <span id="report-from-res-analysis-error" class="text text-danger"></span>
                        
                
                        <div class="clearfix">&nbsp;</div>
                        <label style="font-weight: bold;"> Until: </label>
                        
                        <input type="date" id="report-until-res-analysis" class="form-control" max="<?php echo date('Y-m-d'); ?>" />
                        <span id="report-until-res-analysis-error" class="text text-danger"></span>
                        
                        <div class="clearfix">&nbsp;</div>

                        <button class="btn btn-primary" onclick = "createReservationAnalysisReport()">View Report</button>
                </div>
     ...
     <div class="col-sm-6">
        ...
        <div class="card-body text">
            <h3 class="card-title" style="text-align: center;">Reservation By Year</h3>
            <div class="clearfix">&nbsp;</div>
            <div class="row">
            <div class="col-sm-6">
            <label style="font-weight:bold; text-align:left"> From: </label>
            <select id="years_from" name="years_from" class="form-control">
                <?php
                for($i=0;$i<=10;$i++){
                    $Current_year=date('Y');
                    $year= $Current_year -$i; ?>
            ...
            <span id="years_from_error" class="text text-danger"></span>
            </div>
                        
            <div class="col-sm-6"> 
                        
                        <label style="font-weight: bold;"> Until: </label> 
                        <select id="years_until" name="years_until" class="form-control">
                            <?php
                            for($i=0;$i<=10;$i++){
                                $Current_year=date('Y');
                                $year= $Current_year -$i; ?>
                        ...
                        <span id="years_until_error" class="text text-danger"></span>
            </div>         
                        <div class="clearfix">&nbsp;</div>
                        </br>
            ...
            <button class="btn btn-primary" onclick="reservationCount()">View Report</button>
        </div>
        </div>
        </div>
    </div>
    ...
    <!--Local Stuff-->
    <script>
        function createReservationAnalysisReport(){
            var fromDate = $("#report-from-res-analysis").val();
            var untilDate = $("#report-until-res-analysis").val();
            if(fromDate == ""){
                $("#report-from-res-analysis-error").text("From date cannot be empty!");
                $("#report-from-res-analysis").focus();
                return false;
            }
            if(untilDate == ""){
                $("#report-until-res-analysis-error").text("Until date cannot be empty!");
                $("#report-until-res-analysis").focus();
                return false;
            }
            window.open(`reportpdf.php?type=reservation_analysis&from=${fromDate}&until=${untilDate}`, '_blank');
        }
        function reservationCount(){
            var fromYear = $("#years_from").val();
            var untilYear = $("#years_until").val();
            if(fromYear == ""){
                $("#years_from_error").text("From date cannot be empty!");
                $("#years_from").focus();
                return false;
            }
            if(untilYear == ""){
                $("#years_until_error").text("Until date cannot be empty!");
                $("#years_until").focus();
                return false;
            }
            window.open(`reportpdf.php?type=reservation_analysisByyear&from=${fromYear}&until=${untilYear}`, '_blank');

        }
         
        $("#report-until-res-analysis").change(function(){
            $("#report-until-res-analysis-error").text("");
        });
        $("#report-from-res-analysis").change(function(){
            $("#report-from-res-analysis-error").text("");
        });
     ...
     <!-- <div class="col-sm-6"> 
                        ...
                        <label style="font-weight: bold;"> Until: </label> 
                        <select name="pkg-name" id="pkg-name" class="form-control">
                                                    <option value="">Select Package</option>
                                                    <?php
                                                    //foreach ($packageName as $pkgNames) {
                                                    ?>
                                                        <option value="<?php //echo $pkgNames['pkg_name']; ?>"><?php //echo $pkgNames['pkg_name']; ?></option>
                                                    <?php
