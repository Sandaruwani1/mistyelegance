...
<body>
                        ...
                        <div class="page-header">
                            <div class="page-title">
                                <ol class="breadcrumb text-left">
                                    ...
                                    <li class="active">Report Package</li>
                                </ol>
                            </div>
                        </div>
                    </div>
    ...
    <div class="row">
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <h3 class="card-title" style="text-align: center;">Package Analysis</h3>

                
                    <label style="font-weight:bold; text-align:left"> From: </label>
                        
                        <input type="date" id="report-from-pkg-analysis" class="form-control"  max="<?php echo date('Y-m-d'); ?>" />
                        <span id="report-from-pkg-analysis-error" class="text text-danger"></span>
                
                        <div class="clearfix">&nbsp;</div>
                        <label style="font-weight: bold;"> Until: </label>
                        
                        <input type="date" id="report-until-pkg-analysis" class="form-control" max="<?php echo date('Y-m-d'); ?>" />
                        <span id="report-until-pkg-analysis-error" class="text text-danger"></span>
                        <div class="clearfix">&nbsp;</div>
                        
                        <button class="btn btn-primary" onclick="createPackageAnalysisReport()">View Report</button>
    ...
    <!--Local Stuff-->
    <script>
        function createPackageAnalysisReport(){
            var fromDate = $("#report-from-pkg-analysis").val();
            var untilDate = $("#report-until-pkg-analysis").val();
            if(fromDate == ""){
                $("#report-from-pkg-analysis-error").text("From date cannot be empty!");
                $("#report-from-pkg-analysis").focus();
                return false;
            }
            if(untilDate == ""){
                $("#report-until-pkg-analysis-error").text("Until date cannot be empty!");
                $("#report-until-pkg-analysis").focus();
                return false;
            }
            window.open(`reportpdf.php?type=package_analysis&from=${fromDate}&until=${untilDate}`, '_blank');
        }

        $("#report-until-pkg-analysis").change(function(){
            $("#report-until-pkg-analysis-error").text("");
        });
        $("#report-from-pkg-analysis").change(function(){
            $("#report-from-pkg-analysis-error").text("");
        });
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function(){
            $.post('../ajax/packageajax.php', {
                action: 'getPackagesByReservationCount'
            }, function(response){
                ...
                function drawChart() {
                    ...
                    var data = google.visualization.arrayToDataTable($.parseJSON(response));
