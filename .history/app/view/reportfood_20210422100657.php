...
<body>
                        ...
                        <div class="page-header">
                            <div class="page-title">
                                <ol class="breadcrumb text-left">
                                    ...
                                    <li class="active">Report Food</li>
                                </ol>
                            </div>
                        </div>
                    </div>
        ...
        <div class="col-sm-6">
            <div class="card w-100">
                <div class="card-body">
                    ...
                    <h3 class="card-title" style="text-align: center;">Food Analysis</h3>

                
                    <label style="font-weight:bold; text-align:left"> From: </label>
                        
                        <input type="date" id="report-from-food-analysis" class="form-control"  max="<?php echo date('Y-m-d'); ?>" />
                        <span id="report-from-food-analysis-error" class="text text-danger"></span>
                
                        <div class="clearfix">&nbsp;</div>
                        <label style="font-weight: bold;"> Until: </label>
                        
                        <input type="date" id="report-until-food-analysis" class="form-control" max="<?php echo date('Y-m-d'); ?>" />
                        <span id="report-until-food-analysis-error" class="text text-danger"></span>

                        <div class="clearfix">&nbsp;</div>
                        
                        <button class="btn btn-primary" onclick="createFoodcountReport()">View Report</button>
    ...
    <!--Local Stuff-->
    <script>
        function createFoodcountReport(){
            var fromDate = $("#report-from-food-analysis").val();
            var untilDate = $("#report-until-food-analysis").val();
            if(fromDate == ""){
                $("#report-from-food-analysis-error").text("From date cannot be empty!");
                $("#report-from-food-analysis").focus();
                return false;
            }
            if(untilDate == ""){
                $("#report-until-food-analysis-error").text("Until date cannot be empty!");
                $("#report-until-food-analysis").focus();
                return false;
            }
            window.open(`reportpdf.php?type=food_analysisCount&from=${fromDate}&until=${untilDate}`, '_blank');
        }

        $("#report-until-food-analysis").change(function(){
            $("#report-until-food-analysis-error").text("");
        });
        $("#report-from-food-analysis").change(function(){
            $("#report-from-food-analysis-error").text("");
        });
