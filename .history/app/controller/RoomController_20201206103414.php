...
class RoomController
{
    ...
    function getRoom($roomID)
    {
        ...
        return $this->roomModel->getRoomById($this->con, $roomID);
    }
    function updateRoom($roomUpdatedata)
    {
        if (!$this->roomModel->updateRoom($this->con, $roomUpdatedata)) {
            header("Location: room.php?err");
        }

        header("Location: viewroom.php?room_id=" . $roomUpdatedata['room-id']);
    }
    function changeRoomstatus($roomId)
    {
        $output = $this->roomModel->getRoomstatus($this->con, $roomId);
        if ($output == -1) {
            return -1;
        }
        $newRoomstatus = 0;
        ...
        $is_updated = $this->roomModel->updateRoomstatus($this->con, $roomId, $newRoomstatus);
        if ($is_updated == -1) {
            return -1;
        }
