...
class ReservationController{
    ...
    function editReservation ($arrival_date,$leaving_date,$adults,$children,$cus_id,$pkg_id, $resId){
      ...
      $paymentdata = $this->paymentModel->getPaymentByresId($this->con, $resId);
      if($paymentdata['payment_amount'] >= $advanceamount){

        $reservationUpdated= $this->reservationModel->updateReservation($this->con,$arrival_date,$leaving_date,$availableRoom['room_id'],$amount,$resId);
      }
      if ($paymentdata['payment_amount']< $advanceamount) {
          $reservationUpdated= $this->reservationModel->updateReservation($this->con, $arrival_date,$leaving_date, $availableRoom['room_id'], $amount, $resId);
          ...
          //add payment
           ...
           $isPaymentAdded = $this->paymentModel->addPayment($this->con, $advanceamount ,$resId);
          if(!$isPaymentAdded){
            return -2;
          }
          $this->paymentModel->deletePaymentById($this->con, $paymentdata['payment_id']);
          $response = array($this->con->lastInsertId(), $paymentdata['payment_amount']);
          return $response;
        }
