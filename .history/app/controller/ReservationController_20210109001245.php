...
class ReservationController{
    ...
    function editReservation ($arrival_date,$leaving_date,$adults,$children,$cus_id,$pkg_id, $resId){
      $availableRoom =  $this->reservationModel->editcheckRoomavailability($this->con, $arrival_date,$leaving_date,$pkg_id,$resId);
      if(!$availableRoom){
        return -1;
      }
      $amount = $this->checkDiscountandAmount($arrival_date,$leaving_date,$pkg_id);
      ...
      $paymentdata = $this->getPaymentByresId($resId);
      if($paymentdata['payment_amount'] >= $advanceamount){

        return $this->reservationModel->updateReservation($this->con,$arrival_date,$leaving_date,$adults,$children,$cus_id,$availableRoom['room_id'],$amount,$resId);
      }
      if ($paymentdata['payment_amount']< $advanceamount) {
          $reservationUpdated= $this->reservationModel->updateReservation($this->con, $arrival_date, $leaving_date, $adults, $children, $cus_id, $availableRoom['room_id'], $amount, $resId);
      }
      ...
      //add payment
       ...
       $isPaymentAdded = $this->paymentModel->addPayment($this->con, $advanceamount-$paymentdata['payment_amount'] ,$resId);
      if(!$isPaymentAdded){
        return -1;
      }
      return $this->con->lastInsertId();
    ...
    function getPaymentByresId($resId){
      return $this->paymentModel->getPaymentByresId($this->con, $resId);
    }
