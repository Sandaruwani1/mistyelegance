...
if($_POST['action'] == 'getAllReservations'){
	...
	$table = <<<EOT
 (
	SELECT res.res_id, pkg.pkg_name, r.room_no, cus.cus_id, cus.cus_fname, res.res_date, res.arrival_date, res.leaving_date, res.is_cancelled_by FROM room r, packages pkg,  customer cus, reservation res WHERE res.cus_id=cus.cus_id AND res.room_id=r.room_id AND r.pkg_id=pkg.pkg_id 
 ) temp
EOT;
...
else if($_POST['action'] == 'ongoinReservations'){
	...
	$table = <<<EOT
 (
	SELECT res.res_id, pkg.pkg_name, r.room_no, cus.cus_id, cus.cus_fname, res.res_date, res.arrival_date, res.leaving_date FROM room r, packages pkg,  customer cus, reservation res WHERE res.cus_id=cus.cus_id AND res.room_id=r.room_id AND r.pkg_id=pkg.pkg_id AND full_payment_amount IS NULL AND res_status = 1
 ) temp
EOT;
