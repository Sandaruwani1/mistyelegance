...
if($_POST['action'] == 'getAllReservations'){
	...
	$table = <<<EOT
 (
	SELECT res.res_id, pkg.pkg_name, r.room_no, cus.cus_id, cus.cus_fname, res.res_date, res.arrival_date, res.leaving_date, res.is_cancelled_by FROM room r, packages pkg,  customer cus, reservation res LEFT OUTER JOIN user u ON res.is_cancelled_by=u.user_id WHERE res.cus_id=cus.cus_id AND res.room_id=r.room_id AND r.pkg_id=pkg.pkg_id AND res.res_status=1
 ) temp
EOT;
