...
EOT;
	...
	$table = <<<EOT
 (
	SELECT res.res_id, pkg.pkg_name, r.room_no, CONCAT(cus.cus_fname, ' ', cus.cus_lname) AS cus_name, res.arrival_date, res.leaving_date, res.res_status FROM room r, packages pkg,  customer cus, reservation res WHERE res.cus_id=cus.cus_id AND res.room_id=r.room_id AND r.pkg_id=pkg.pkg_id AND full_payment_amount IS NULL AND res_status = 1
 ) temp
EOT;
