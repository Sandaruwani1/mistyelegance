...
if($_POST['action'] == 'getAllPackages'){
	...
	$table = <<<EOT
 (
	SELECT CASE WHEN (pkg.discount_from <= NOW() AND pkg.discount_until >= NOW()) THEN 1 ELSE 0 END AS is_discount_available, pkg.pkg_name, pkgc.pkg_cat, pkg.rate_per_night, pkg.discount, pkg.discount_from, pkg.discount_until, pkg.pkg_id FROM packages pkg, package_cat pkgc WHERE pkg.pkg_cat_id=pkgc.pkg_cat_id AND pkg.is_deleted=0
 ) temp
EOT;
