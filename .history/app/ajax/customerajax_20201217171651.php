...
EOT;
	...
	$nic = $_POST['nic'];
	echo json_encode($customerController->getCustomer($nic));
}
else if($_POST['action'] == 'checkNicExistence'){
	$nic = $_POST['nic'];
	echo $customerController->checkNicExistence($nic);
}
else if($_POST['action'] == 'addCusFromReservation'){
	$nic = $_POST['nic'];
	$fname = $_POST['fname'];
	$lname = $_POST['lname'];
	$email = $_POST['email'];
	...
	echo json_encode($customerController->addCustomerfromReservation($nic,$fname,$lname,$email,$tel,$dob,$country,$gender));
}
