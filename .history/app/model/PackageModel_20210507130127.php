...
class PackageModel{
	...
	function getTodaydiscounts($con){
		...
		$stmt = $con->prepare("SELECT pkg_name, discount_rate FROM packages WHERE (SELECT MAX(discount_rate) FROM packages)  AND now() BETWEEN discount_from AND discount_until");
