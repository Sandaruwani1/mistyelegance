...
class CustomerModel{
	...
	function OngoingresData($con,$cus_id){
		...
		$stmt = $con->prepare("SELECT * FROM reservation rs, payment p, room rm, packages pkg WHERE p.res_id=rs.res_id AND rs.cus_id=? AND rm.pkg_id=pkg.pkg_id AND rs.full_payment_amount IS NULL");
	...
	function ReshistoryData($con,$cus_id){
		...
		$stmt = $con->prepare("SELECT * FROM (SELECT rs.*, rm.room_no, pkg.pkg_name, pkg.pkg_id FROM reservation rs, room rm, packages pkg 
