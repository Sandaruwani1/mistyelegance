...
if($_POST['action'] == 'getAllFeedbacks'){
	...
	$table = <<<EOT
 (
    SELECT f.feedback_id, pkg.pkg_name, CONCAT(cus.cus_fname, ' ', cus.cus_lname) AS cus_name,  f.feedback, f.rating FROM feedback_and_ratings f ,packages pkg , customer cus WHERE  f.pkg_id=pkg.pkg_id AND f.cus_id=cus.cus_id
 ) temp
EOT;
