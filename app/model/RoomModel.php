...
class RoomModel
{
	...
	function getRoomById($con, $roomId){
		...
		$stmt = $con->prepare("SELECT * FROM room WHERE room_id=?");
	...
	function getRoom($con,$room_id){
		...
		$stmt = $con->prepare("SELECT * FROM packages pkg , room r WHERE pkg.pkg_id=r.pkg_id AND room_id=?");
	...
	function updateRoom($con,$roomUpdatedata){
		if($con->errorCode() != null){
			if($con->errorCode() != '00000'){
				return false;
			}
		...
		$pkg_name=$roomUpdatedata['pkg-id'];
		$room_id= $roomUpdatedata['room-id'];
		
		$stmt = $con->prepare("UPDATE room SET pkg_id=? WHERE room_id=?");
		$stmt->execute(array($pkg_name, $room_id));
	...
	function getRoomstatus($con,$roomId){
		...
		$stmt = $con->prepare("SELECT room_status FROM room WHERE room_id=? ");
		$stmt->execute(array($roomId));
		$row = $stmt->fetch(PDO::FETCH_ASSOC);
	...
	function updateRoomstatus($con,$roomId,$newRoomstatus){
		...
		if($con->errorCode() != null){
			...
			$stmt = $con->prepare("UPDATE room SET room_status=? WHERE room_id=?");

			$stmt->execute(array($newRoomstatus,$roomId));
